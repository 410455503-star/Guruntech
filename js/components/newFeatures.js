    ...
    <div class="card">
      <div class="card-header" style="display: flex; justify-content: space-between; align-items: center;">
        ...
        <div style="display: flex; gap: 12px; align-items: center;">
          <label style="font-size: 13px; color: var(--text-secondary);">工程阶段筛选：</label>
          <select class="form-input" id="progressPhaseFilter" onchange="filterProgressByPhase(this.value)" style="width: 160px; padding: 6px 12px; font-size: 13px;">
...
function renderProgressTableBody(progressReports, state) {
  ...
  return `
    <table class="table">
      ...
      <tbody>
        ${filtered.map(report => {
          const task = store.getTaskById(report.taskId);
          const project = store.getProjectById(report.projectId);
          const reporter = store.getUserById(report.reporterId);
          const statusColors = {
            normal: 'tag-success',
            delayed: 'tag-danger',
            ahead: 'tag-info'
          ...
          return `
            <tr>
              <td>${DateUtils.formatDate(report.date)}</td>
              <td>${task?.name || '-'}</td>
              <td>${project?.name || '-'}</td>
              <td>${phaseNames[project?.phase] || '-'}</td>
              <td>${report.plannedProgress}%</td>
              <td>${report.actualProgress}%</td>
              <td><span class="tag ${statusColors[report.status]}">${statusNames[report.status]}</span></td>
              <td>${reporter?.name || '-'}</td>
              <td>
...
function filterProgressByPhase(phase) {
  const state = store.getState();
  const progressReports = state.progressReports || [];
      ...
      <div class="modal-content" style="max-width: 600px;">
        ...
        <div class="modal-body">
          <div style="display: grid; gap: 16px;">
            ...
            <div style="display: flex; justify-content: space-between;">
              <span class="text-muted">计划进度</span>
              <span>${report.plannedProgress}%</span>
            </div>
            <div style="display: flex; justify-content: space-between;">
              <span class="text-muted">实际进度</span>
              <span>${report.actualProgress}%</span>
            </div>
            <div style="display: flex; justify-content: space-between;">
              <span class="text-muted">状态</span>
              <span><span class="tag tag-${report.status === 'delayed' ? 'danger' : report.status === 'ahead' ? 'info' : 'success'}">${statusNames[report.status]}</span></span>
            </div>
            <div style="display: flex; justify-content: space-between;">
            ...
            ${report.description ? `
              <div>
                <span class="text-muted">进度说明</span>
                <p style="margin-top: 8px; padding: 12px; background: var(--bg-light); border-radius: 4px;">${report.description}</p>
              </div>
            ` : ''}
          </div>
        </div>
            ...
            <table class="table">
              ...
              <tbody>
                ${warnings.map(warning => {
                  ...
                  return `
                    <tr>
                      <td>${DateUtils.formatDate(warning.dueDate)}</td>
                      <td>${warning.message}</td>
                      <td><span class="tag ${levelColors[warning.level]}">${levelNames[warning.level]}</span></td>
                      <td><span class="tag ${warning.isRead ? 'tag-success' : 'tag-warning'}">${warning.isRead ? '已处理' : '未处理'}</span></td>
                      <td>
                        ${!warning.isRead ? `
...
function markWarningAsRead(warningId) {
  store.markWarningAsRead(warningId);
          ...
          <table class="table">
            ...
            <tbody>
              ${dailyLogs.map(log => {
                ...
                const reporter = store.getUserById(log.reporterId);
                const statusColors = {
                  normal: 'tag-success',
                  delayed: 'tag-warning',
                  stopped: 'tag-danger'
                ...
                return `
                  <tr>
                    <td>${DateUtils.formatDate(log.date)}</td>
                    <td>${log.weather}</td>
                    <td>${project?.name || '-'}</td>
                    <td>${phaseNames[log.phase] || '-'}</td>
                    <td>${log.workers}人</td>
                    <td><span class="tag ${statusColors[log.status]}">${statusNames[log.status]}</span></td>
                    <td>${log.equipmentStatus ? `<span class="tag ${equipmentStatusColors[log.equipmentStatus]}">${equipmentStatusNames[log.equipmentStatus]}</span>` : '-'}</td>
                    <td><span class="tag ${performanceColors[log.performance]}">${performanceNames[log.performance]}</span></td>
                    <td>${reporter?.name || '-'}</td>
                    <td>
          ...
          <div style="display: grid; gap: 16px;">
            ...
            <div class="grid grid-cols-3 gap-16">
              <div>
                ...
                <p style="font-weight: 500;">${log.workers}人</p>
              </div>
              <div>
                <span class="text-muted">状态</span>
                <p style="font-weight: 500;">${statusNames[log.status]}</p>
              </div>
              <div>
                <span class="text-muted">绩效</span>
                <p style="font-weight: 500;">${performanceNames[log.performance]}</p>
              </div>
            </div>
            <div class="grid grid-cols-2 gap-16">
              <div>
                <span class="text-muted">工程阶段</span>
                <p style="font-weight: 500;">${phaseNames[log.phase] || '-'}</p>
              </div>
              <div>
                <span class="text-muted">设备运行状态</span>
                <p style="font-weight: 500;">${equipmentStatusNames[log.equipmentStatus] || '-'}</p>
              </div>
            </div>
            ${log.content ? `
              <div>
...
function exportDailyLogs() {
  ...
  dailyLogs.forEach(log => {
    const project = projects.find(p => p.id === log.projectId);
    const reporter = store.getUserById(log.reporterId);
    const statusNames = { normal: '正常', delayed: '滞后', stopped: '停工' };
    const performanceNames = { excellent: '优秀', good: '良好', average: '一般', poor: '较差' };
    const phaseNames = { preparation: '前期准备', civil: '土建施工', mechanical: '机电安装', commissioning: '工艺调试', 'trial-run': '试运行', acceptance: '竣工验收' };
    const equipmentStatusNames = { normal: '正常', fault: '故障', maintenance: '维护中' };
    csvContent += `${log.date},${log.weather},${project?.name || '-'},${phaseNames[log.phase] || '-'},${log.workers},${statusNames[log.status]},${equipmentStatusNames[log.equipmentStatus] || '-'},${performanceNames[log.performance]},${reporter?.name || '-'}\n`;
  });
  ...
  const html = `
    ...
    <div class="stats-grid" style="margin-bottom: 24px;">
      <div class="stat-card">
        ...
        <div class="stat-card-value">¥${totalBudget.toLocaleString()}</div>
        <div class="stat-card-label">总预算</div>
      </div>
      <div class="stat-card">
        <div class="stat-card-header">
          <div class="stat-card-icon yellow"><i class="fas fa-chart-line"></i></div>
        </div>
        <div class="stat-card-value">¥${totalSpent.toLocaleString()}</div>
        <div class="stat-card-label">已使用</div>
      </div>
      <div class="stat-card">
        <div class="stat-card-header">
          <div class="stat-card-icon green"><i class="fas fa-piggy-bank"></i></div>
        </div>
        <div class="stat-card-value">¥${remaining.toLocaleString()}</div>
        <div class="stat-card-label">剩余可用</div>
      </div>
      <div class="stat-card">
        <div class="stat-card-header">
          <div class="stat-card-icon ${overBudget.length > 0 ? 'red' : 'green'}"><i class="fas fa-exclamation-circle"></i></div>
        </div>
        <div class="stat-card-value" style="color: ${overBudget.length > 0 ? '#ef4444' : ''}">${overBudget.length}</div>
        <div class="stat-card-label">超预算项</div>
      </div>
    </div>
    ...
    <div class="card" style="margin-bottom: 24px;">
      <div class="card-header">
        <h3 class="card-title">项目预算概览</h3>
      </div>
      <div class="card-body">
        ${projects.length === 0 ? `
          <div class="empty-state" style="padding: 40px 0;">
            <i class="fas fa-folder-open" style="font-size: 40px; color: var(--text-muted); opacity: 0.3;"></i>
            <p class="text-muted" style="margin-top: 12px;">暂无项目，请先创建项目</p>
          </div>
        ` : projects.map(project => {
          ...
          return `
            <div style="padding: 20px; margin-bottom: 16px; background: var(--bg-primary); border-radius: 12px; border: 1px solid var(--border-color); ${isOver ? 'border-left: 4px solid #ef4444;' : isWarn ? 'border-left: 4px solid #f59e0b;' : 'border-left: 4px solid #3b82f6;'}">
              <div style="display: flex; justify-content: space-between; align-items: flex-start; flex-wrap: wrap; gap: 16px; margin-bottom: 16px;">
                <div>
                  <div style="display: flex; align-items: center; gap: 8px; margin-bottom: 4px;">
                    ...
                    <span class="tag tag-muted" style="font-size: 11px;">${projectBudgets.length}项预算</span>
                    ${isOver ? '<span class="tag tag-danger" style="font-size: 11px;">超预算</span>' : isWarn ? '<span class="tag tag-warning" style="font-size: 11px;">接近超支</span>' : (projTotal > 0 ? '<span class="tag tag-success" style="font-size: 11px;">正常</span>' : '')}
                  </div>
                  <span style="font-size: 12px; color: var(--text-muted);">${project.description || ''}</span>
                </div>
                ${projTotal > 0 ? `
                <div style="text-align: right;">
                  <div style="font-size: 22px; font-weight: 700; color: ${isOver ? '#ef4444' : isWarn ? '#f59e0b' : 'var(--text-primary)'};">¥${projTotal.toLocaleString()}</div>
              ...
              ${projTotal > 0 ? `
                ...
                <div style="display: grid; grid-template-columns: repeat(4, 1fr); gap: 12px; margin-bottom: 14px;">
                  <div style="text-align: center; padding: 10px 8px; background: rgba(59, 130, 246, 0.06); border-radius: 8px;">
                    <div style="font-size: 16px; font-weight: 700; color: #3b82f6;">¥${projTotal.toLocaleString()}</div>
                    <div style="font-size: 11px; color: var(--text-muted);">总预算</div>
                  </div>
                  <div style="text-align: center; padding: 10px 8px; background: rgba(245, 158, 11, 0.06); border-radius: 8px;">
                    <div style="font-size: 16px; font-weight: 700; color: #f59e0b;">¥${projSpent.toLocaleString()}</div>
                    <div style="font-size: 11px; color: var(--text-muted);">已使用</div>
                  </div>
                  <div style="text-align: center; padding: 10px 8px; background: rgba(16, 185, 129, 0.06); border-radius: 8px;">
                    <div style="font-size: 16px; font-weight: 700; color: #10b981;">¥${projRemaining.toLocaleString()}</div>
                    <div style="font-size: 11px; color: var(--text-muted);">剩余</div>
                  </div>
                  <div style="text-align: center; padding: 10px 8px; background: rgba(99, 102, 241, 0.06); border-radius: 8px;">
                    <div style="font-size: 16px; font-weight: 700; color: ${isOver ? '#ef4444' : isWarn ? '#f59e0b' : '#6366f1'};">${projRate}%</div>
                ...
                <div style="margin-bottom: 12px;">
                  <div class="progress-bar" style="height: 10px; border-radius: 5px;">
                    <div class="progress-bar-fill" style="width: ${Math.min(projRate, 100)}%; border-radius: 5px; background: ${isOver ? '#ef4444' : isWarn ? '#f59e0b' : 'linear-gradient(90deg, #3b82f6, #6366f1)'};"></div>
                  </div>
                </div>
                
                <!-- 类别明细行 -->
                ${Object.keys(projCategories).length > 0 ? `
                <div style="display: flex; flex-wrap: wrap; gap: 8px; padding-top: 10px; border-top: 1px dashed var(--border-color);">
                  ${Object.entries(projCategories).map(([cat, s]) => {
                    const catRate = s.total > 0 ? Math.round((s.spent / s.total) * 100) : 0;
                    return `
                      <span style="padding: 3px 10px; background: var(--bg-secondary); border-radius: 20px; font-size: 12px;">
                        <span style="color: var(--text-secondary);">${cat}</span>
                        <span style="font-weight: 600; margin-left: 4px;">¥${s.total.toLocaleString()}</span>
                        <span style="color: var(--text-muted); margin-left: 2px;">(${catRate}%)</span>
                      </span>
                    `;
                  }).join('')}
                </div>
              ...
              ` : `
                <div style="padding: 16px 0; text-align: center; color: var(--text-muted); font-size: 13px;">
                  <i class="fas fa-plus-circle" style="color: #3b82f6; margin-right: 6px;"></i>
                  暂未设置预算，点击右上角"新增预算项"为此项目添加预算
                </div>
              `}
            </div>
          `;
    ...
    <div class="card">
      <div class="card-header" style="display: flex; justify-content: space-between; align-items: center; flex-wrap: wrap; gap: 12px;">
        ...
        <div style="display: flex; gap: 12px; flex-wrap: wrap;">
          <select class="form-input" id="budgetProjectFilter" onchange="filterBudgetItems()" style="width: 180px; padding: 6px 10px; font-size: 13px;">
            <option value="">全部项目</option>
            ${projects.map(p => `<option value="${p.id}">${p.name}</option>`).join('')}
          </select>
          <select class="form-input" id="budgetCategoryFilter" onchange="filterBudgetItems()" style="width: 150px; padding: 6px 10px; font-size: 13px;">
            <option value="">全部类别</option>
            ${categories.map(cat => `<option value="${cat}">${cat}</option>`).join('')}
          </select>
        </div>
      </div>
      <div class="card-body" id="budgetTableContainer">
...
function renderBudgetTableBody(budgetList, projects) {
  ...
  Object.keys(grouped).forEach(pid => {
    ...
    html += `
      <div style="margin-bottom: 20px; ${Object.keys(grouped).indexOf(pid) > 0 ? 'margin-top: 24px;' : ''}">
        <div style="display: flex; align-items: center; gap: 12px; margin-bottom: 12px; padding: 8px 12px; background: var(--bg-primary); border-radius: 8px;">
          <i class="fas fa-folder-open" style="color: #3b82f6; font-size: 16px;"></i>
          <span style="font-weight: 600; font-size: 15px;">${projectName}</span>
          <span class="tag tag-muted" style="font-size: 11px;">${group.items.length}项预算</span>
          <span style="margin-left: auto; font-size: 13px; color: var(--text-secondary);">
            合计: <b>¥${groupTotal.toLocaleString()}</b> / 已用: <b style="color: #f59e0b;">¥${groupSpent.toLocaleString()}</b>
          </span>
        </div>
        <table class="table table-striped">
          <thead>
            <tr>
              ...
              <th>预算名称</th>
              <th style="width: 120px;">预算金额</th>
              <th style="width: 120px;">已用金额</th>
              <th style="width: 120px;">剩余金额</th>
          ...
          <tbody>
            ${group.items.map(item => {
              ...
              return `
                <tr class="${usageRate >= 100 ? 'bg-danger-light' : ''}">
                  <td><span class="tag tag-info" style="font-size: 11px;">${item.category || '其他'}</span></td>
                  <td>
                    <div style="font-weight: 500;">${item.name}</div>
                    ${item.description ? `<div style="font-size: 12px; color: var(--text-muted); margin-top: 2px;">${item.description}</div>` : ''}
                  </td>
                  <td>¥${itemBudget.toLocaleString()}</td>
                  <td>¥${itemSpent.toLocaleString()}</td>
                  <td style="color: ${remainingAmount < 0 ? '#ef4444' : ''};">¥${remainingAmount.toLocaleString()}</td>
                  <td>
                    <div style="display: flex; align-items: center; gap: 8px;">
                      <div class="progress-bar" style="width: 100px; height: 8px; border-radius: 4px;">
                        <div class="progress-bar-fill ${usageRate >= 100 ? 'bg-danger' : usageRate >= 80 ? 'bg-warning' : ''}" style="width: ${Math.min(usageRate, 100)}%; border-radius: 4px;"></div>
                      </div>
                      <span style="font-size: 13px; font-weight: 500;">${usageRate}%</span>
                    </div>
                  </td>
                  <td><span class="tag ${statusColor}" style="font-size: 11px;">${statusText}</span></td>
                  <td>
                    <div style="display: flex; gap: 4px;">
                      <button class="btn btn-secondary btn-sm" onclick="showEditBudgetItemModal('${item.id}')" title="编辑">
                        <i class="fas fa-edit"></i>
                      </button>
                      <button class="btn btn-secondary btn-sm" onclick="showAddExpenseModal('${item.id}')" title="记录支出">
                        <i class="fas fa-plus"></i>
                      </button>
                      <button class="btn btn-danger btn-sm" onclick="deleteBudgetItem('${item.id}')" title="删除">
...
function deleteBudgetItem(itemId) {
  ...
  const state = store.getState();
  const budgets = state.budgets.filter(b => b.id !== itemId);
  store.setState({ budgets: budgets });
  renderContent();
  ...
  const html = `
    ...
    <div class="stats-grid" style="margin-bottom: 24px;">
      <div class="stat-card">
        ...
        <div class="stat-card-value">¥${totalAmount.toLocaleString()}</div>
        <div class="stat-card-label">总费用金额</div>
        <div class="stat-card-trend">${payments.length} 条记录</div>
      </div>
      <div class="stat-card">
        <div class="stat-card-header">
          <div class="stat-card-icon green"><i class="fas fa-check-circle"></i></div>
        </div>
        <div class="stat-card-value">¥${completedAmount.toLocaleString()}</div>
        <div class="stat-card-label">已支付</div>
        <div class="stat-card-trend">${totalAmount > 0 ? ((completedAmount / totalAmount) * 100).toFixed(1) : 0}%</div>
      </div>
      <div class="stat-card">
        <div class="stat-card-header">
        ...
        <div class="stat-card-trend">${totalAmount > 0 ? ((pendingAmount / totalAmount) * 100).toFixed(1) : 0}%</div>
      </div>
      <div class="stat-card">
        <div class="stat-card-header">
        ...
        <div class="stat-card-label">费用记录</div>
      </div>
    </div>
    
    <div class="card" style="margin-bottom: 24px;">
      <div class="card-header">
        <h3 class="card-title">项目费用概览</h3>
      </div>
      <div class="card-body">
        ${projects.length === 0 ? `
          <div class="empty-state" style="padding: 40px 0;">
              ...
              <div style="display: flex; justify-content: space-between; align-items: flex-start; flex-wrap: wrap; gap: 16px; margin-bottom: 16px;">
                <div>
                  <div style="display: flex; align-items: center; gap: 8px; margin-bottom: 4px;">
                    <i class="fas fa-folder-open" style="color: #3b82f6;"></i>
                    <span style="font-weight: 600; font-size: 16px;">${project.name}</span>
                    ${projectPayments.length > 0 ? '<span class="tag tag-muted" style="font-size: 11px;">' + projectPayments.length + '条</span>' : '<span class="tag tag-muted" style="font-size: 11px;">无记录</span>'}
                    ${projPending > 0 ? '<span class="tag tag-warning" style="font-size: 11px;">有待支付</span>' : (projTotal > 0 ? '<span class="tag tag-success" style="font-size: 11px;">已结清</span>' : '')}
                  </div>
                </div>
                ${projTotal > 0 ? '<div style="text-align: right;"><div style="font-size: 22px; font-weight: 700;">¥' + projTotal.toLocaleString() + '</div><div style="font-size: 12px; color: var(--text-muted);">费用合计</div></div>' : ''}
              </div>
              ${projTotal > 0 ? `
                <div style="display: grid; grid-template-columns: repeat(4, 1fr); gap: 12px; margin-bottom: 14px;">
                  <div style="text-align: center; padding: 10px 8px; background: rgba(59, 130, 246, 0.06); border-radius: 8px;">
                    <div style="font-size: 16px; font-weight: 700; color: #3b82f6;">¥${projTotal.toLocaleString()}</div>
                    <div style="font-size: 11px; color: var(--text-muted);">总费用</div>
                  </div>
                  <div style="text-align: center; padding: 10px 8px; background: rgba(16, 185, 129, 0.06); border-radius: 8px;">
                    <div style="font-size: 16px; font-weight: 700; color: #10b981;">¥${projCompleted.toLocaleString()}</div>
                    <div style="font-size: 11px; color: var(--text-muted);">已支付</div>
                  </div>
                  <div style="text-align: center; padding: 10px 8px; background: rgba(245, 158, 11, 0.06); border-radius: 8px;">
                    <div style="font-size: 16px; font-weight: 700; color: #f59e0b;">¥${projPending.toLocaleString()}</div>
                    <div style="font-size: 11px; color: var(--text-muted);">待支付</div>
                  </div>
                  <div style="text-align: center; padding: 10px 8px; background: rgba(99, 102, 241, 0.06); border-radius: 8px;">
                    <div style="font-size: 16px; font-weight: 700; color: #6366f1;">${projRate}%</div>
                    <div style="font-size: 11px; color: var(--text-muted);">支付率</div>
                  </div>
                </div>
                <div style="margin-bottom: 12px;">
                  <div class="progress-bar" style="height: 10px; border-radius: 5px;">
                    <div class="progress-bar-fill" style="width: ${Math.min(projRate, 100)}%; border-radius: 5px; background: linear-gradient(90deg, #3b82f6, #10b981);"></div>
                  </div>
                </div>
                ${Object.keys(projCategories).length > 0 ? '<div style="display: flex; flex-wrap: wrap; gap: 8px; padding-top: 10px; border-top: 1px dashed var(--border-color);">' + Object.entries(projCategories).map(([cat, amt]) => '<span style="padding: 3px 10px; background: var(--bg-secondary); border-radius: 20px; font-size: 12px;"><span style="color: var(--text-secondary);">' + cat + '</span><span style="font-weight: 600; margin-left: 4px;">¥' + amt.toLocaleString() + '</span></span>').join('') + '</div>' : ''}
              ` : '<div style="padding: 16px 0; text-align: center; color: var(--text-muted); font-size: 13px;"><i class="fas fa-plus-circle" style="color: #3b82f6; margin-right: 6px;"></i>暂未登记费用</div>'}
            </div>
          `;
        }).join('')}
      </div>
    ...
    <div class="card">
      <div class="card-header" style="display: flex; justify-content: space-between; align-items: center; flex-wrap: wrap; gap: 12px;">
        ...
        <div style="display: flex; gap: 12px; flex-wrap: wrap;">
          <select class="form-input" id="paymentProjectFilter" onchange="filterPayments()" style="width: 180px; padding: 6px 10px; font-size: 13px;">
            <option value="all">全部项目</option>
            ${projects.map(p => '<option value="' + p.id + '">' + p.name + '</option>').join('')}
          </select>
          <select class="form-input" id="paymentCategoryFilter" onchange="filterPayments()" style="width: 150px; padding: 6px 10px; font-size: 13px;">
            <option value="all">全部类别</option>
            ${categories.map(c => '<option value="' + c + '">' + c + '</option>').join('')}
          </select>
          <select class="form-input" id="paymentStatusFilter" onchange="filterPayments()" style="width: 120px; padding: 6px 10px; font-size: 13px;">
      ...
      <div class="card-body" id="paymentTableContainer">
      ...
      <div class="modal-content" style="max-width: 500px;">
        ...
        <div class="modal-body">
          <form id="addPaymentForm">
            ...
            <div class="form-group">
              <label class="form-label">备注</label>
              <textarea class="form-input form-textarea" name="remark" placeholder="请输入备注"></textarea>
            </div>
          </form>
        </div>
        <div class="modal-footer">
...
function renderPaymentTableBody(payments, projects) {
  ...
  Object.keys(grouped).forEach(pid => {
    ...
    html += '<div style="margin-bottom: 20px;">' +
      '<div style="display: flex; align-items: center; gap: 12px; margin-bottom: 12px; padding: 8px 12px; background: var(--bg-primary); border-radius: 8px;">' +
        '<i class="fas fa-folder-open" style="color: #3b82f6; font-size: 16px;"></i>' +
        '<span style="font-weight: 600; font-size: 15px;">' + (group.project?.name || '未分配项目') + '</span>' +
        '<span class="tag tag-muted" style="font-size: 11px;">' + group.items.length + '条</span>' +
        '<span style="margin-left: auto; font-size: 13px; color: var(--text-secondary);">合计: <b>¥' + groupTotal.toLocaleString() + '</b></span>' +
      '</div>' +
      '<table class="table table-striped"><thead><tr>' +
        '<th style="width: 110px;">日期</th><th style="width: 100px;">类别</th><th style="width: 130px;">金额</th><th>合同号</th><th style="width: 110px;">支付日期</th><th style="width: 80px;">状态</th><th>备注</th>' +
      '</tr></thead><tbody>' +
      group.items.map(p => '<tr>' +
        '<td>' + DateUtils.formatDate(p.date) + '</td>' +
        '<td><span class="tag tag-info" style="font-size: 11px;">' + (p.category || '其他') + '</span></td>' +
        '<td style="font-weight: 600; color: #3b82f6;">¥' + p.amount.toLocaleString() + '</td>' +
        '<td>' + (p.contractNo || '-') + '</td>' +
        '<td>' + DateUtils.formatDate(p.dueDate) + '</td>' +
        '<td><span class="tag ' + (statusColors[p.status] || 'tag-muted') + '" style="font-size: 11px;">' + (statusNames[p.status] || p.status) + '</span></td>' +
        '<td style="max-width: 150px; overflow: hidden; text-overflow: ellipsis; white-space: nowrap;">' + (p.remark || '-') + '</td>' +
      '</tr>').join('') +
      '</tbody></table></div>';
  });
...
function renderMaterials() {
  ...
  const html = `
    ...
    <div class="grid grid-cols-1 md:grid-cols-4 gap-24 mb-24">
      <div class="card">
        <div class="text-sm text-muted mb-8">总材料</div>
        <div class="text-3xl font-bold text-primary">${materials.length}</div>
      </div>
      <div class="card">
        <div class="text-sm text-muted mb-8">已下单</div>
        <div class="text-3xl font-bold text-info">${ordered}</div>
      </div>
      <div class="card">
        <div class="text-sm text-muted mb-8">运输中</div>
        <div class="text-3xl font-bold text-warning">${inTransit}</div>
      </div>
      <div class="card">
        <div class="text-sm text-muted mb-8">已到货</div>
        <div class="text-3xl font-bold text-success">${received}</div>
      </div>
    </div>
    
    <div class="card">
      <div class="card-header">
        <h3 class="card-title">材料列表</h3>
      </div>
      <div class="card-body">
        ${materials.length === 0 ? `
          <div class="empty-state" style="padding: 40px;">
            <i class="fas fa-boxes" style="font-size: 48px; color: var(--text-muted);"></i>
            <h3 style="margin-top: 16px;">暂无材料记录</h3>
            <p class="text-muted">点击"新增材料"按钮创建第一条记录</p>
          </div>
        ` : `
          <table class="table">
            <thead>
            ...
            <tbody>
              ${materials.map(material => {
                ...
                return `
                  <tr>
                    <td>${DateUtils.formatDate(material.date)}</td>
                    <td>${project?.name || '-'}</td>
                    <td>${material.name}</td>
                    <td>${material.category || '-'}</td>
                    <td>${material.quantity}</td>
                    <td>¥${material.unitPrice.toLocaleString()}</td>
                    <td>¥${(material.quantity * material.unitPrice).toLocaleString()}</td>
                    <td><span class="tag ${statusColors[material.status]}">${statusNames[material.status]}</span></td>
                    <td>
                      <button class="btn btn-secondary btn-sm" onclick="viewMaterial('${material.id}')">
                        <i class="fas fa-eye"></i>
                      </button>
      ...
      <div class="modal-content" style="max-width: 500px;">
        ...
        <div class="modal-body">
          <form id="addMaterialForm">
            ...
            <div class="form-group">
              <label class="form-label">备注</label>
              <textarea class="form-input form-textarea" name="notes" placeholder="请输入备注"></textarea>
            </div>
          </form>
        </div>
        <div class="modal-footer">
...
function renderAfterSale() {
  ...
  const html = `
    ...
    <div class="card">
      <div class="card-header">
        <h3 class="card-title">售后工单列表</h3>
        <div style="display: flex; gap: 8px;">
          <button class="btn btn-secondary btn-sm" onclick="afterSaleFilter = {search: '', projectId: '', status: '', priority: ''}; renderContent()">
            <i class="fas fa-sync-alt"></i> 重置
          </button>
        </div>
      </div>
      <div class="card-body">
        <div style="display: grid; grid-template-columns: 250px 150px 120px 120px auto; gap: 12px; margin-bottom: 16px; align-items: center;">
          <input 
            type="text" 
            class="form-input" 
            placeholder="搜索问题描述..." 
          ...
          <span class="text-muted" style="font-size: 12px;">共 ${filteredAfterSales.length} 条记录</span>
        </div>
        
        ${filteredAfterSales.length === 0 ? `
          <div class="empty-state" style="padding: 40px;">
            <i class="fas fa-headset" style="font-size: 48px; color: var(--text-muted);"></i>
            <h3 style="margin-top: 16px;">暂无售后记录</h3>
            <p class="text-muted">点击"新增售后"按钮创建第一条记录</p>
          </div>
        ` : `
    ...
    html += `
      <div style="margin-bottom: 20px;">
        <div style="display: flex; align-items: center; gap: 12px; margin-bottom: 12px; padding: 8px 12px; background: var(--bg-primary); border-radius: 8px;">
          ...
          <span style="font-weight: 600; font-size: 15px;">${group.project?.name || '未分配项目'}</span>
          <span class="tag tag-muted" style="font-size: 11px;">${group.items.length}条</span>
          ${pending > 0 ? `<span class="tag tag-warning" style="font-size: 11px;">${pending}条待处理</span>` : '<span class="tag tag-success" style="font-size: 11px;">全部完成</span>'}
        </div>
        <table class="table table-striped">
          <thead>
            <tr>
              <th style="width: 110px;">日期</th>
              <th style="width: 90px;">问题类别</th>
              <th style="width: 90px;">负责人</th>
              <th>问题描述 / 客户</th>
              <th style="width: 70px;">优先级</th>
              <th style="width: 90px;">计划完成</th>
              <th style="width: 80px;">状态</th>
          ...
          <tbody>
            ${group.items.map(afterSale => {
              ...
              return `
                <tr style="${isOverdue ? 'background: rgba(239, 68, 68, 0.04);' : ''}">
                  <td>${DateUtils.formatDate(afterSale.date)}</td>
                  <td><span class="tag tag-info" style="font-size: 11px;">${afterSale.issueType || '其他'}</span></td>
                  <td>${assignee?.name || '<span class="text-muted">未分配</span>'}</td>
                  <td>
                    <div style="max-width: 250px;">
                      <span style="cursor: pointer; color: #3b82f6;" onclick="viewAfterSale('${afterSale.id}')">
                        ${afterSale.description.substring(0, 35)}${afterSale.description.length > 35 ? '...' : ''}
                      </span>
                      ${afterSale.clientName ? `<div style="font-size: 11px; color: var(--text-muted);">客户: ${afterSale.clientName}</div>` : ''}
                    </div>
                  </td>
                  <td><span class="tag ${priorityColors[afterSale.priority]}" style="font-size: 11px;">${priorityNames[afterSale.priority]}</span></td>
                  <td style="font-size: 13px; ${isOverdue ? 'color: #ef4444; font-weight: 600;' : ''}">
                    ${dueDate ? DateUtils.formatDate(afterSale.dueDate) + (isOverdue ? ' (超期)' : '') : '-'}
                  </td>
                  <td>
                    <select class="form-input" style="padding: 2px 6px; font-size: 11px; width: 90px;" 
        ...
        <div class="modal-body">
          <form id="addAfterSaleForm">
            ...
            <div class="form-group">
              ...
              <textarea class="form-input form-textarea" name="description" required placeholder="请描述售后问题"></textarea>
            </div>
            <div class="form-group">
            ...
            <div class="form-group">
              ...
              <textarea class="form-input form-textarea" name="result" placeholder="请输入处理结果"></textarea>
            </div>
          </form>
        </div>
        <div class="modal-footer">
      ...
      <div class="modal-content" style="max-width: 500px;">
        ...
        <div class="modal-body">
          <form id="editAfterSaleForm">
            ...
            <div class="form-group">
              <label class="form-label">问题描述 <span style="color: red;">*</span></label>
              <textarea class="form-input form-textarea" name="description" required>${afterSale.description}</textarea>
            </div>
            <div class="form-group">
              <label class="form-label">优先级 <span style="color: red;">*</span></label>
              <select class="form-input" name="priority" required>
            ...
            <div class="form-group">
              <label class="form-label">处理结果</label>
              <textarea class="form-input form-textarea" name="result">${afterSale.result || ''}</textarea>
            </div>
          </form>
        </div>
        <div class="modal-footer">
...
function renderTemporaryWorkers() {
  ...
  const html = `
    ...
    <div class="card">
      <div class="card-header">
        <h3 class="card-title">临时用工列表</h3>
      </div>
      <div class="card-body">
        ${workers.length === 0 ? `
          <div class="empty-state" style="padding: 40px;">
            <i class="fas fa-user-plus" style="font-size: 48px; color: var(--text-muted);"></i>
            <h3 style="margin-top: 16px;">暂无临时用工记录</h3>
            <p class="text-muted">点击"新增用工"按钮创建第一条记录</p>
          </div>
        ` : `
          <table class="table">
            <thead>
                  ...
                  <tr>
                    <td>
                      <div style="display: flex; align-items: center; gap: 10px;">
                        <div style="width: 32px; height: 32px; border-radius: 50%; background: linear-gradient(135deg, var(--primary-color), var(--purple-color)); display: flex; align-items: center; justify-content: center; color: white; font-weight: 600; font-size: 12px;">
                    ...
                    <td>${worker.trade || '-'}</td>
                    <td>${project?.name || '-'}</td>
                    <td>${worker.startDate ? DateUtils.formatDate(worker.startDate) : '-'}</td>
                    <td>${worker.totalHours || 0}</td>
                    <td>${worker.hourlyRate || 0}</td>
                    <td><span class="tag ${statusColors[worker.status]}">${statusNames[worker.status]}</span></td>
                    <td>
                      <button class="btn btn-secondary btn-sm" onclick="showWorkerAttendanceModal('${worker.id}')">
                        <i class="fas fa-clock"></i>
                      </button>
                      <button class="btn btn-secondary btn-sm" onclick="viewTemporaryWorker('${worker.id}')">
                        <i class="fas fa-eye"></i>
                      </button>
                      <button class="btn btn-secondary btn-sm" onclick="editTemporaryWorker('${worker.id}')">
                        <i class="fas fa-edit"></i>
                      </button>
                      <button class="btn btn-danger btn-sm" onclick="deleteTemporaryWorkerConfirm('${worker.id}')">
                        <i class="fas fa-trash"></i>
                      </button>
                    </td>
                  </tr>
        ...
        <div class="modal-body">
          <div style="display: flex; align-items: center; gap: 16px; margin-bottom: 24px; padding-bottom: 24px; border-bottom: 1px solid var(--border-color);">
            ...
            <div>
              ...
              <p class="text-muted">${worker.trade} | ${statusNames[worker.status]}</p>
            </div>
          </div>
