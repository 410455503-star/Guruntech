// header.js 模块级命名空间（用于缓存事件处理器引用，防止重复绑定）
const headerJs = {};
...
function renderHeader() {
  ...
  // 构建面包屑导航
  const breadcrumb = [{ label: '首页', route: 'dashboard', active: false }];
  ...
  if (state.currentRoute !== 'dashboard') {
    if (state.currentProjectId && projectRoutes.includes(state.currentRoute)) {
      ...
      breadcrumb.push({ label: '项目管理', route: 'projects', active: false });
      breadcrumb.push({ label: project ? project.name : '项目详情', route: 'projects', projectId: state.currentProjectId, active: false });
      breadcrumb.push({ label: pageTitle, route: state.currentRoute, projectId: state.currentProjectId, active: true });
    } else {
      breadcrumb.push({ label: pageTitle, route: state.currentRoute, active: true });
    }
  } else {
    breadcrumb.push({ label: pageTitle, route: 'dashboard', active: true });
  }
  
  // getInitials 和 getRoleName 使用 sidebar.js 中的全局定义，不再重复定义
  ...
  const html = `
      ...
      <div class="header-title-group">
        ...
        <div class="header-breadcrumb">
          ${breadcrumb.map((part, index) => {
            const clickHandler = !part.active ? `onclick="navigateToBreadcrumb('${part.route}'${part.projectId ? `, '${part.projectId}'` : ''})"` : '';
            const style = !part.active ? 'cursor: pointer;' : '';
            return index > 0
              ? `<i class="fas fa-chevron-right breadcrumb-separator"></i><span class="breadcrumb-item${part.active ? ' active' : ''}" ${clickHandler} style="${style}">${part.label}</span>`
              : `<span class="breadcrumb-item${part.active ? ' active' : ''}" ${clickHandler} style="${style}">${part.label}</span>`;
          }).join('')}
        </div>
      </div>
    ...
    <div class="header-right">
      ...
      <div class="header-icon-btn cloud-sync-btn" id="cloudSyncBtn" title="云端同步" onclick="showCloudSyncDropdown()">
        <span class="cloud-icon-wrapper">
          <i class="fas fa-cloud" id="cloudIcon"></i>
          <div class="sync-dot" id="syncDot"></div>
        </span>
        <span class="sync-status-text" id="syncStatusText">本地</span>
      </div>
...
function attachHeaderEvents() {
  ...
  if (notificationBtn && notificationList) {
    // 移除旧监听器（防止路由切换时重复绑定）
    if (headerJs._notifyClickHandler) {
      notificationBtn.removeEventListener('click', headerJs._notifyClickHandler);
    }
    if (headerJs._docClickHandler) {
      document.removeEventListener('click', headerJs._docClickHandler);
    }

    headerJs._notifyClickHandler = (e) => {
      e.stopPropagation();
      notificationList.classList.toggle('active');
    };
    notificationBtn.addEventListener('click', headerJs._notifyClickHandler);

    headerJs._docClickHandler = (e) => {
      if (!notificationList.contains(e.target) && !notificationBtn.contains(e.target)) {
        notificationList.classList.remove('active');
      }
    };
    document.addEventListener('click', headerJs._docClickHandler);
...
function initCloudSyncUI() {
  if (typeof cloudSync !== 'undefined' && cloudSync) {
    ...
    // 若已配置但显示离线，主动触发重新连接测试
    if (cloudSync.isConfigured && (cloudSync.syncStatus === 'offline' || cloudSync.syncStatus === 'error')) {
      setTimeout(async () => {
...
// 显示云端同步下拉菜单
function showCloudSyncDropdown() {
  // 等待 cloudSync 初始化
  if (typeof cloudSync === 'undefined') {
    console.warn('[CloudSync] 等待初始化...');
    setTimeout(showCloudSyncDropdown, 100);
    return;
  }
  
  const existingDropdown = document.getElementById('cloudSyncDropdown');
  if (existingDropdown) {
  ...
  const html = `
    <div class="cloud-sync-dropdown" id="cloudSyncDropdown">
      <div class="cloud-sync-header">
        <i class="fas fa-cloud"></i>
        <span>云端同步</span>
        <span class="cloud-sync-status" id="cloudSyncStatusText">${cloudSync && cloudSync.isConfigured ? '已连接' : '未配置'}</span>
      </div>
      <div class="cloud-sync-divider"></div>
      <div class="user-dropdown-item" style="font-size: 12px; color: #6b7280; cursor: default; padding: 6px 16px;">
        <i class="fas fa-sync-alt"></i>
        <span>实时同步：${cloudSync && cloudSync._pollInterval ? '已开启（5秒间隔）' : '未开启'}</span>
      </div>
      <div class="cloud-sync-divider"></div>
      <div class="user-dropdown-item" onclick="cloudSync.syncToCloud(); closeCloudSyncDropdown();">
        <i class="fas fa-cloud-upload-alt"></i>
        <span>同步到云端</span>
      </div>
      <div class="user-dropdown-item" onclick="cloudSync.refreshFromCloud(); closeCloudSyncDropdown();">
        <i class="fas fa-cloud-download-alt"></i>
        <span>从云端刷新（合并）</span>
      </div>
      <div class="user-dropdown-item" onclick="cloudSync.forcePullFromCloud(); closeCloudSyncDropdown();">
        <i class="fas fa-sync"></i>
        <span>强制从云端拉取（覆盖本地）</span>
      </div>
      <div class="cloud-sync-divider"></div>
      <div class="user-dropdown-item" onclick="cloudSync.diagnoseSync(); closeCloudSyncDropdown();">
        <i class="fas fa-stethoscope"></i>
        <span>诊断同步状态</span>
      </div>
      <div class="user-dropdown-item" onclick="cloudSync.openSettings(); closeCloudSyncDropdown();">
  ...
  const dropdown = document.getElementById('cloudSyncDropdown');
  if (cloudSyncBtn && dropdown) {
    const rect = cloudSyncBtn.getBoundingClientRect();
    // 使用 fixed 定位
  ...
  setTimeout(() => {
    document.addEventListener('click', closeCloudSyncDropdownOnOutsideClick);
  }, 0);
}

function closeCloudSyncDropdown() {
  const dropdown = document.getElementById('cloudSyncDropdown');
  if (dropdown) {
  ...
  document.removeEventListener('click', closeCloudSyncDropdownOnOutsideClick);
}

function closeCloudSyncDropdownOnOutsideClick(e) {
  const cloudSyncBtn = document.getElementById('cloudSyncBtn');
  const dropdown = document.getElementById('cloudSyncDropdown');
  if (dropdown && cloudSyncBtn && !dropdown.contains(e.target) && !cloudSyncBtn.contains(e.target)) {
    closeCloudSyncDropdown();
  }
}
...
function navigateToBreadcrumb(route, projectId) {
  if (!route) return;
