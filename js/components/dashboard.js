...
// getStatusName 由 app.js 全局提供，不再重复定义
...
function getPriorityName(priority) {
  ...
  return names[priority] || priority || '未设置';
}
...
function renderDashboard() {
  ...
  const html = `
    ...
    <div style="margin-top: 24px;">
      <div class="card" style="border-radius: 16px; box-shadow: 0 4px 20px rgba(0, 0, 0, 0.06); border: 1px solid #e2e8f0; overflow: hidden;">
        <div class="card-header" style="display: flex; justify-content: space-between; align-items: center; padding: 20px 24px; border-bottom: 1px solid #f1f5f9; background: linear-gradient(135deg, #f8fafc 0%, #f1f5f9 100%);">
          <div style="display: flex; align-items: center; gap: 12px;">
            <div style="width: 8px; height: 24px; background: linear-gradient(180deg, #3b82f6 0%, #6366f1 100%); border-radius: 4px;"></div>
            <h3 style="font-size: 18px; font-weight: 600; color: #1e293b;">项目健康概览</h3>
          </div>
          <button class="btn btn-secondary btn-sm" style="padding: 8px 16px; border-radius: 8px; background: rgba(59, 130, 246, 0.1); color: #3b82f6; border: 1px solid rgba(59, 130, 246, 0.2);" onclick="router.navigate('projects'); renderContent();">
      ...
      <div style="display: flex; align-items: start; gap: 12px; padding: 12px; border-radius: 8px; margin-bottom: 12px; background: var(--bg-light);">
        ...
        <div style="flex: 1; min-width: 0;">
          ...
          <div style="display: flex; align-items: center; gap: 8px; font-size: 12px;">
            ...
            ${isOverdue ? '<span class="tag tag-red"><i class="fas fa-exclamation-triangle"></i> 逾期</span>' : ''}
...
function renderProjectHealthOverview(projects) {
  ...
  function getHealthStatus(project) {
    if (!project) return { status: 'unknown', label: '未知', color: 'gray' };
    const progress = store.getProjectProgress(project.id);
    ...
    // 安全处理日期
    let start = null, end = null, totalDuration = 0;
    try {
    ...
    if (!start || !end || totalDuration <= 0) return { status: 'behind', label: '风险', color: 'red' };
      ...
      <table style="width: 100%; border-collapse: collapse; font-size: 14px;">
        <thead>
          <tr style="background: #f8fafc;">
            <th style="text-align: left; padding: 16px 24px; font-weight: 600; color: #64748b; font-size: 13px; width: 269px; border-bottom: 2px solid #e2e8f0;">项目名称</th>
            <th style="text-align: center; padding: 16px 24px; font-weight: 600; color: #64748b; font-size: 13px; width: 200px; border-bottom: 2px solid #e2e8f0;">进度</th>
            <th style="text-align: left; padding: 16px 24px; font-weight: 600; color: #64748b; font-size: 13px; border-bottom: 2px solid #e2e8f0;">截止日期</th>
            <th style="text-align: center; padding: 16px 24px; font-weight: 600; color: #64748b; font-size: 13px; width: 200px; border-bottom: 2px solid #e2e8f0;">健康状态</th>
          </tr>
        </thead>
        <tbody>
          ${projects.slice(0, 8).map((project, index) => {
            ...
            return `
              <tr style="border-bottom: 1px solid #f1f5f9; cursor: pointer; transition: all 0.2s ease;" 
                  onmouseenter="this.style.background='rgba(59, 130, 246, 0.03)'"
                  onmouseleave="this.style.background='transparent'"
                  onclick="store.setState({ currentRoute: 'projects', currentProjectId: '${project.id}' }); router.navigate('projects/${project.id}'); renderContent();">
                <td style="padding: 18px 24px;">
                  <div style="display: flex; align-items: center; gap: 12px;">
                    <div style="width: 10px; height: 10px; border-radius: 50%; background: ${healthColorMap[health.color]}; box-shadow: 0 0 6px ${healthColorMap[health.color]}40;"></div>
                    <div style="font-weight: 500; color: #1e293b; font-size: 14px;">${project.name}</div>
                  </div>
                </td>
                <td style="padding: 18px 24px;">
                  <div style="display: flex; align-items: center; gap: 14px; justify-content: center;">
                    <div style="flex: 1; max-width: 150px; height: 10px; background: #e2e8f0; border-radius: 5px; overflow: hidden;">
                      <div style="height: 100%; width: ${progress}%; border-radius: 5px; background: linear-gradient(90deg, ${progressColor}, ${progress === 100 ? '#10b981' : progressColor}); transition: width 0.5s ease; box-shadow: 0 2px 4px ${progressColor}30;"></div>
                    </div>
                    <span style="font-size: 15px; font-weight: 700; color: ${progressColor}; min-width: 50px; text-align: right;">${progress}%</span>
                  </div>
                </td>
                <td style="padding: 18px 24px;">
                  <div style="display: flex; align-items: center; gap: 10px;">
                    <div style="width: 36px; height: 36px; border-radius: 10px; background: rgba(59, 130, 246, 0.1); display: flex; align-items: center; justify-content: center;">
                      <i class="fas fa-calendar" style="font-size: 14px; color: #3b82f6;"></i>
                    </div>
                    <span style="color: #64748b; font-size: 14px;">${DateUtils.formatDate(project.endDate)}</span>
                  </div>
                </td>
                <td style="padding: 18px 24px; text-align: center;">
                  <span style="display: inline-flex; align-items: center; gap: 8px; padding: 8px 16px; border-radius: 20px; font-size: 13px; font-weight: 500; 
                    background: ${healthColorMap[health.color]}15; color: ${healthColorMap[health.color]}; box-shadow: 0 2px 8px ${healthColorMap[health.color]}10;">
                    <span style="width: 10px; height: 10px; border-radius: 50%; background: ${healthColorMap[health.color]}; box-shadow: 0 0 6px ${healthColorMap[health.color]}50;"></span>
                    ${health.label}
                  </span>
                </td>
              </tr>
