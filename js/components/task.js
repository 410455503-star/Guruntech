...
function renderKanbanBoard(tasks) {
  const columns = {
    ...
    paused: { title: '暂停', class: 'paused', tasks: tasks.filter(t => t.status === 'paused') },
    terminated: { title: '终止', class: 'terminated', tasks: tasks.filter(t => t.status === 'terminated') },
  ...
  return `
    <div class="kanban-card ${task.parentId ? 'has-parent-task' : ''} ${isSelected ? 'selected' : ''}" draggable="true" data-task-id="${task.id}" ondragstart="handleDragStart(event)" ondragend="handleDragEnd(event)" style="border-left: 4px solid ${priorityBorderColor};">
      ...
      <div style="display: flex; gap: 4px; margin-bottom: 12px; flex-wrap: wrap;">
        ${task.status !== 'todo' ? `<button class="btn btn-xs btn-todo" onclick="event.stopPropagation(); quickChangeTaskStatus('${task.id}', 'todo')" title="移至待开始"><i class="fas fa-arrow-left"></i> 待开始</button>` : ''}
        ${task.status !== 'in_progress' ? `<button class="btn btn-xs btn-progress" onclick="event.stopPropagation(); quickChangeTaskStatus('${task.id}', 'in_progress')" title="移至进行中"><i class="fas fa-play"></i> 进行中</button>` : ''}
        ${task.status !== 'paused' ? `<button class="btn btn-xs btn-paused" onclick="event.stopPropagation(); quickChangeTaskStatus('${task.id}', 'paused')" title="暂停任务"><i class="fas fa-pause"></i> 暂停</button>` : ''}
        ${task.status !== 'terminated' ? `<button class="btn btn-xs btn-terminated" onclick="event.stopPropagation(); quickChangeTaskStatus('${task.id}', 'terminated')" title="终止任务"><i class="fas fa-stop"></i> 终止</button>` : ''}
        ${task.status !== 'completed' ? `<button class="btn btn-xs btn-completed" onclick="event.stopPropagation(); quickChangeTaskStatus('${task.id}', 'completed')" title="标记完成"><i class="fas fa-check"></i> 完成</button>` : ''}
      </div>
            ...
            <div class="grid grid-cols-2">
              ...
              <div class="form-group">
                ...
                <select class="form-select" name="status">
                  ...
                  <option value="terminated" ${task.status === 'terminated' ? 'selected' : ''}>终止</option>
