...
async function getEzvizAccessToken() {
  ...
  try {
    // 通过服务端代理获取token，密钥不暴露在前端
    const response = await fetch(`${EZVIZ_CONFIG.apiUrl}/token`, {
      method: 'POST',
    ...
    if (videoUrl && videoUrl.hls) {
      ...
      loading.innerHTML = `
        <div style="text-align: center;">
          ...
          <div style="font-size: 12px; color: #9ca3af; margin-top: 4px;">请在"萤石云配置"中启用视频功能</div>
        </div>
      `;
    }
  }
...
// 显示萤石云配置弹窗（简化版，密钥由服务端管理）
function showEzvizConfigModal() {
  const enabled = Storage.get('ezviz_enabled', false);
      ...
      <div style="padding: 24px;">
        <div style="padding: 16px; background: #eff6ff; border-radius: 8px; margin-bottom: 20px;">
          <div style="font-size: 13px; color: #1e40af; line-height: 1.6;">
            <i class="fas fa-info-circle" style="margin-right: 8px;"></i>
            萤石云API密钥由服务端配置管理。如需修改，请联系管理员设置环境变量。<br>
            前端只需启用/禁用视频功能。
          </div>
        </div>
        ...
        <div style="display: flex; align-items: center; gap: 12px; margin-bottom: 20px;">
          <label style="display: flex; align-items: center; gap: 8px; cursor: pointer;">
            <input type="checkbox" id="ezvizEnabled" style="width: 18px; height: 18px;" ${enabled ? 'checked' : ''}>
...
// 初始化时加载萤石云配置（服务端管理，无需加载密钥）
function initEzvizConfig() {
  // 密钥由服务端通过环境变量管理，前端无需加载
}
