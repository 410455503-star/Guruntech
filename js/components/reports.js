...
function renderDailyReport(tasks, resources) {
  ...
  const todayTasks = tasks.filter(t => t.startDate <= todayStr && (!t.dueDate || t.dueDate >= todayStr));
  const completedToday = tasks.filter(t => t.status === 'completed');
  const inProgress = tasks.filter(t => t.status === 'in_progress');
  const todoTasks = tasks.filter(t => t.status === 'todo');
  
  const completedResources = resources.filter(r => r.status === 'installed').length;
  const installingResources = resources.filter(r => r.status === 'installing').length;
          ...
          return `
            <div class="task-progress-item">
              ...
              <div class="task-progress-footer">
                ...
                <span class="text-muted">截止: ${task.dueDate}</span>
              </div>
            </div>
          `;
        }).join('')}
...
function renderWeeklyReport(tasks, resources) {
  ...
  const avgProgress = weeklyTasks.length > 0 
    ? Math.round(weeklyTasks.reduce((sum, t) => sum + t.progress, 0) / weeklyTasks.length) 
    : 0;
  ...
  return `
    ...
    <div>
      ...
      <div class="completed-tasks">
        ${weeklyTasks.filter(t => t.status === 'completed').slice(0, 5).map(task => `
          <div class="completed-task-item">
            <i class="fas fa-check-circle" style="color: var(--success-color); margin-right: 12px;"></i>
            <div>
              <div>${task.name}</div>
              <div class="text-muted text-sm">${task.dueDate}</div>
            </div>
          </div>
        `).join('')}
      </div>
...
function renderMonthlyReport(tasks, resources) {
  ...
  const totalBudget = resources.reduce((sum, r) => sum + (r.totalPrice || 0), 0);
  const spentBudget = resources.filter(r => r.status === 'installed').reduce((sum, r) => sum + (r.totalPrice || 0), 0);
      ...
      <div>
        ...
        <div class="budget-progress">
          ...
          <div class="budget-bar">
            <div class="budget-fill" style="width: ${(spentBudget / totalBudget * 100).toFixed(0)}%;"></div>
          </div>
          <div class="budget-info">
            <span>已使用</span>
            <span>¥${(spentBudget / 10000).toFixed(1)}万 (${(spentBudget / totalBudget * 100).toFixed(1)}%)</span>
          </div>
        </div>
      </div>
      ...
      <div>
        ...
        <div class="resource-status">
          <div class="resource-status-item">
            <div class="resource-status-bar installed" style="width: 75%;"></div>
            <span>已安装 (75%)</span>
          </div>
          <div class="resource-status-item">
            <div class="resource-status-bar installing" style="width: 25%;"></div>
            <span>安装中 (25%)</span>
          </div>
        </div>
      </div>
...
function attachReportEvents() {
  ...
  viewButtons.forEach(btn => {
    btn.addEventListener('click', () => {
      ...
      if (view === 'daily') {
        content.innerHTML = renderDailyReport(state.tasks, state.resources);
      } else if (view === 'weekly') {
        content.innerHTML = renderWeeklyReport(state.tasks, state.resources);
      } else if (view === 'monthly') {
        content.innerHTML = renderMonthlyReport(state.tasks, state.resources);
      }
    });
  });
}
