...
// 根据概率和影响程度计算风险级别
function computeRiskLevel(probability, impact) {
  const scoreMap = { 'high': 3, 'medium': 2, 'low': 1 };
  const score = (scoreMap[probability] || 0) * (scoreMap[impact] || 0);
...
function handleCreateRisk() {
  ...
  const riskData = {
    ...
    level: computeRiskLevel(formData.get('probability'), formData.get('impact')),
