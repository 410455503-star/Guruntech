  ...
  const html = `
    ...
    <nav class="sidebar-nav">
      ...
      <div class="nav-group">
        ...
        <div class="nav-item ${state.currentRoute === 'video' ? 'active' : ''}" data-route="video">
          <i class="fas fa-video"></i>
          <span>视频监控</span>
          ${(state.cameras || []).length > 0 ? `<span class="nav-badge">${(state.cameras || []).length}</span>` : ''}
        </div>
      </div>
      ...
      <div class="nav-group">
        ...
        <div class="nav-item ${state.currentRoute === 'warnings' ? 'active' : ''}" data-route="warnings">
          <i class="fas fa-bell"></i>
          <span>工期预警</span>
          ${(state.warnings || []).filter(w => !w.isRead).length > 0 ? `<span class="nav-badge badge-danger">${(state.warnings || []).filter(w => !w.isRead).length}</span>` : ''}
        </div>
      </div>
      ...
      <div class="nav-group">
        ...
        <div class="nav-item ${state.currentRoute === 'settings' ? 'active' : ''}" data-route="settings">
