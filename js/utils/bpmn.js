class BPMNEngine {
  ...
  evaluateCondition(condition, task) {
    ...
    const safeProperties = ['progress', 'status', 'isCompleted', 'isOverdue', 'hasParent', 'priority', 'assignee', 'tags'];
      ...
      while (i < expr.length) {
        ...
        if ('<>!=&|!()'.includes(char)) {
          ...
          if (char === '=' && expr[i + 1] === '=' && expr[i + 2] === '=') {
            tokens.push('===');
            i += 3;
          } else if (char === '!' && expr[i + 1] === '=' && expr[i + 2] === '=') {
            tokens.push('!==');
            i += 3;
    ...
    const parseValue = (token) => {
      ...
      if (token.startsWith('"') && token.endsWith('"')) return token.slice(1, -1);
      if (safeProperties.includes(token)) return ctx[token];
      return token;
    };
    ...
    const precedence = { '!': 4, '>': 3, '<': 3, '>=': 3, '<=': 3, '==': 2, '!=': 2, '===': 2, '!==': 2, '&&': 1, '||': 0, 'in': 2 };
