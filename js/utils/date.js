const DateUtils = {
  ...
  formatRelativeTime(date) {
    ...
    if (diff < 0) return this.formatDate(date);
  ...
  formatDuration(startDate, endDate, unit = 'auto') {
  ...
  getDurationInWeeks(startDate, endDate) {
    const days = this.getDaysBetween(startDate, endDate);
    if (days <= 0) return 0;
    return { weeks: Math.floor(days / 7), days: days % 7, totalWeeks: days / 7 };
  },
  
  getDurationInMonths(startDate, endDate) {
    const days = this.getDaysBetween(startDate, endDate);
    if (days <= 0) return 0;
    return { months: Math.floor(days / 30), days: days % 30, totalMonths: days / 30 };
  }
};
