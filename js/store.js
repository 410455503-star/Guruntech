class Store {
  ...
  getDefaultTaskStatusConfig() {
    return [
      { id: 'todo', name: '待开始', color: '#6b7280', icon: 'fa-circle', category: 'pending', editable: false },
      { id: 'in_progress', name: '进行中', color: '#3b82f6', icon: 'fa-play', category: 'active', editable: false },
      { id: 'paused', name: '暂停', color: '#f59e0b', icon: 'fa-pause', category: 'active', editable: false },
      { id: 'delayed', name: '滞后', color: '#f97316', icon: 'fa-alarm-clock', category: 'active', editable: false },
      { id: 'overdue', name: '逾期', color: '#ef4444', icon: 'fa-exclamation-triangle', category: 'active', editable: false },
      { id: 'terminated', name: '终止', color: '#6b7280', icon: 'fa-stop', category: 'closed', editable: false },
      { id: 'completed', name: '已完成', color: '#10b981', icon: 'fa-check-circle', category: 'closed', editable: false },
      { id: 'cancelled', name: '已取消', color: '#9ca3af', icon: 'fa-times-circle', category: 'closed', editable: false }
    ];
  }
  ...
  checkTaskDependencies(taskId) {
    ...
    const allBlockedBy = [...bpmnResult.blockedBy, ...incompleteDependencies.map(dep => ({
      ...
      reason: `传统依赖：前置任务"${dep.name}"尚未完成`
    }))];
  ...
  updateTaskStatus(taskId, status) {
    ...
    if (status === 'in_progress' && task) {
      ...
      if (!dependencyCheck.canStart) {
        ...
        throw new Error(`无法开始此任务，前置条件未满足：\n\n${reasons}`);
      }
    }
  ...
  markAllWarningsAsRead() {
    this.state.warnings.forEach(w => w.isRead = true);
    this.saveToStorage();
    this.notify();
  }
