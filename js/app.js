...
document.addEventListener('click', function(e) {
  ...
  while (target && target !== document.body) {
    if (target.hasAttribute('onclick')) {
      ...
      // 扩展正则表达式以匹配更多模态框函数
      const modalFunctionMatch = onclickValue.match(/(showCreate\w+Modal|showEdit\w+Modal|showAdd\w+Modal|show\w+Modal)\s*\(/);
      if (modalFunctionMatch) {
        const functionName = modalFunctionMatch[1];
        
        e.preventDefault();
        e.stopPropagation();
        
        console.log('Modal function:', functionName, 'exists:', typeof window[functionName] === 'function');
        
        if (typeof window[functionName] === 'function') {
          const argsMatch = onclickValue.match(/\(([^)]*)\)/);
          const args = argsMatch ? argsMatch[1].split(',').map(arg => {
            arg = arg.trim();
          ...
          console.error('Please check if the function is properly defined and loaded before this script.');
        }
        break;
      }
    }
...
function showModal(modalId, contentHtml) {
  ...
  console.log('Modal content length:', contentHtml.length);
...
function testModal() {
  alert('testModal called!');
  showModal('testModal', `
    <div class="modal-content">
      <div class="modal-header">
        <h3 class="modal-title">测试模态框</h3>
        <button class="btn btn-secondary btn-sm" onclick="closeModal('testModal')">
      ...
      <div class="modal-footer">
        <button class="btn btn-secondary" onclick="closeModal('testModal')">关闭</button>
      </div>
    </div>
  `);
}
